...
class PlayState extends MusicBeatState
{
	...
	public var pressedThree:Bool;
	public var needToSeeNotes:Bool;
	public var spaceToDodge:FlxText;
	
	public var MADTRAILAAAAAAAA:FlxTrail;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'steady':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/steady/steadyDialogue'));
			case 'natural':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/natural/naturalDialogue'));
			case 'apocalyptic':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/apocalyptic/apocalypticDialogue'));
		}
		...
		if (SONG.stage == null)
		{
			switch (storyWeek)
			{
				...
				case 7:
					if (songLowercase == 'apocalyptic' || songLowercase == 'madness')
					{
						stageCheck = 'nighttime';
					}
					else
					{
						stageCheck = 'park';
			...
			switch (stageCheck)
			{
				...
				case 'schoolEvil':
					...
					case 'park':
					{
						curStage = 'park';
						defaultCamZoom = 0.8;
						var bg:FlxSprite = new FlxSprite( -453, -481).loadGraphic(Paths.image('bg', 'weekt'));
						bg.antialiasing = true;
						bg.active = false;
						bg.setGraphicSize(Std.int(bg.width * 0.8));
						bg.updateHitbox();
						...
						var ground:FlxSprite = new FlxSprite( -887, 52).loadGraphic(Paths.image('BackGroundF', 'weekt'));
					...
					case 'nighttime':
					{
						...
						var bg:FlxSprite = new FlxSprite( -453, -481).loadGraphic(Paths.image('park/night', 'weekt'));
						bg.antialiasing = true;
						bg.active = false;
						bg.setGraphicSize(Std.int(bg.width * 0.8));
						bg.updateHitbox();
						add(bg);
						bg.scrollFactor.set(0.8);
						
						var ground:FlxSprite = new FlxSprite( -887, 52).loadGraphic(Paths.image('park/APOCBG', 'weekt'));
						ground.antialiasing = true;
						ground.active = false;
						ground.setGraphicSize(Std.int(ground.width * 0.8));
						ground.updateHitbox();
						...
						var UFO:FlxSprite = new FlxSprite( -513, -545).loadGraphic(Paths.image('park/UFONew', 'weekt'));
						UFO.antialiasing = true;
						UFO.active = false;
						UFO.setGraphicSize(Std.int(UFO.width * 0.8));
						UFO.updateHitbox();
						add(UFO);
					}
					case 'ufo':
					{
						...
						var bg:FlxSprite = new FlxSprite(-710, -310).loadGraphic(Paths.image('ufo/UFOBG', 'UNIDENTIFIED'));
						bg.antialiasing = true;
						bg.active = false;
						bg.setGraphicSize(Std.int(bg.width * 0.8));
						bg.updateHitbox();
		...
		if (curSong == 'steady' || curSong == 'natural' || curSong == 'apocalyptic' || curSong == 'madness')
		{	
			healthBar.createFilledBar(0xFF00D4FF, 0xFF66FF33);			
		}
		else
		{
		...
		kadeEngineWatermark = new FlxText(4, healthBarBG.y
			...
			+ (Main.watermarks ? " | Vs Teel Mod ver1.1 | Kade Engine " + MainMenuState.kadeEngineVer : ""), 16);
	...
	{
		if (curSong == 'apocalyptic')
		{
			spaceToDodge = new FlxText(0, 300, 0, "Press 3 to if you understand" + "\nPress SPACE to make girlfriend protect you for a certain amount of time" + "\nIf your screen flashes red you've been hit" + "\nIf your hear two noises on the third beat you need to press space" + "e", 22);
			spaceToDodge.color = FlxColor.RED;
			spaceToDodge.setBorderStyle(FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK, 1, 1);
			spaceToDodge.alignment = FlxTextAlign.CENTER;
			spaceToDodge.cameras = [camHUD];
			spaceToDodge.screenCenter();
			add(spaceToDodge);
			new FlxTimer().start(14, function(tmr:FlxTimer)
			{
				if (!pressedThree)
				{
					FlxTween.tween(spaceToDodge, {y:1200}, 3, {ease:FlxEase.backIn});
					pressedThree = true;
				}
			});
		}
					...
					switch (daNote.noteType) //you can add as many cases as you want, just make sure the noteType number matches properly!
							{
								...
								case 1: //example of noteType that can't be misses (warning note)
								{
									gotHit = true;
									stunTime = 0.05;
									noteMiss(daNote.noteData, daNote);
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				{
					if (SONG.song.toLowerCase() == 'apocalyptic')
					{
						#if windows
						DiscordClient.changePresence("Finished Playing Week 1 of Vs Teel",null,null,null,null);
						#end
						FlxG.switchState(new VideoState('assets/videos/cutmov.webm', new FreeplayState()));
					}
	...
	override function beatHit()
	{
		...
		if (PlayState.SONG.song.toLowerCase() == 'steady')
		{
			if (curBeat == 136)
			{
				remove(dad);
				iconP2.animation.play('teel');
				dad = new Character(100, 250, 'teel');
				add(dad);
			}
		}
		
		if (PlayState.SONG.song.toLowerCase() == 'apocalyptic')
		{
			if (curBeat == 376)
			{
				defaultCamZoom = 1;
			...
			{
				remove(dad);
				iconP2.animation.play('lost-it-teel');
				dad = new Character(100, 250, 'lost-it-teel');
				MADTRAILAAAAAAAA = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
				add(MADTRAILAAAAAAAA);
				add(dad);
			}
			if (curBeat == 126)
			{
				FlxG.sound.play(Paths.sound('200bpmwarning', 'shared'), 1);
			}
			if (curBeat == 158)
			{
				FlxG.sound.play(Paths.sound('200bpmwarning', 'shared'), 1);
			...
			if (curBeat == 128)
			{
				if (!isInvulnerable)
				{
					...
					onScratched(1);		
					FlxG.sound.play(Paths.sound('IMHIT'), 1);
				}
				else
				{
					onDodge();
	...
	function onScratched(stunTimeSet:Float)
	{
		FlxG.camera.flash(FlxColor.RED, 1);
		gotHit = true;
		stunTime = stunTimeSet;
		boyfriend.playAnim('scratched', true);
		camHUD.alpha = 0.75;
		new FlxTimer().start(0.5, function(tmr:FlxTimer)
		{
			camHUD.alpha = 1;
